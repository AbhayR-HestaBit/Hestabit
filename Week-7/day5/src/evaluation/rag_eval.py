...
class RAGEvaluator:
    # scores how good the ai answer is based on the provided context
    def __init__(
        self,
    ...
    def faithfulness_score(self, answer: str, context: str) -> float:
        # checks if the answer uses fabricated facts not found in the context
    ...
    def context_relevance_score(self, query: str, context: str) -> float:
        # measures if the retrieved context actually matches the user question
        texts = [(query or "").strip(), (context or "").strip()]
        embeddings = self.model.encode(texts)
        sim = cosine_similarity(
            np.array([embeddings[0]]), np.array([embeddings[1]])
        )[0][0]
        # cosine similarity can be outside [0,1]; normalise
